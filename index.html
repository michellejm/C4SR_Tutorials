...
You will need to set up a (free) account on Carto at [www.carto.com](www.carto.com). You only need an email and password, the questions are optional. If you have GitHub, I encourage you to sign up using your GitHub account so that any files you store (or update) on GitHub can be easily imported into your map.

Once you set up an account, you will be taken to the page to Add Datasets. We are going to be working with some Census Data about language in New York City. In the next tutorial, we will work with income and social services as well. 

First we need to decide what kind of boundaries we are going to use. For this tutorial, we will use Census Tracts (click here for more on the different types). 
...
The data we are using for this tutorial is pre-cleaned and formatted. We will have another session on finding and cleaning data.

1. Import file to demark census tracts in New York City
This file originally came from [the US Census](https://www.census.gov/geo/maps-data/data/cbf/cbf_tracts.html), though it is available in multiple locations, multiple ways:
	1. Download and open this [ZIP FILE](https://github.com/michellejm/C4SR_Tutorials/blob/master/Data_Carto_1.zip)
	2. Open "gz_2010_36.csv" 
